...
A single‑file React component you can drop into a fresh Vite + Tailwind project to get a beautiful, content‑ready site for Bloom with Dignity CBO.
...
3) Replace `src/App.jsx` with the component below

- Put the required images into `public/assets` (or `src/assets` and update paths accordingly).
...
- `logo.svg` (optional; inline SVG fallback included in the component)
- `miss-director-1.jpg`
- `miss-director-2.jpg`
- `miss-director-3.jpg`
...
Use the logo colours (teal/green/gold/soft‑lavender). Red remains in the logo but is not used as a primary UI colour. The component uses Tailwind defaults and an inline gold colour token.
    ...
    <section id="home" className="py-16 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
      <div>
        ...
        <div className="mt-8 flex gap-6 text-sm text-gray-600">
          ...
          <div><strong className="text-[#F7C81C]">2</strong><div>Programs</div></div>
...
- Deploy the `dist/` folder or connect your repository in Netlify for automatic deploys.

### Notes

- If you prefer a named Tailwind colour for gold, extend your Tailwind theme:
...
module.exports = {
  ...
  theme: {
    extend: {
      colors: { gold: { 500: '#F7C81C' } },
    },
  },
}
```

Then you can use `text-gold-500` (the component above currently uses `text-[#F7C81C]`).
